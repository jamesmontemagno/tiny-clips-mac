...
class CaptureManager: ObservableObject {
            ...
            do {
                ...
                if let currentURL = savedVideoURL,
                   ...
                   !capturedMouseClickData.events.isEmpty {
                    savedVideoURL = try await overlayMouseClicksOnVideo(
            ...
            do {
                ...
                if settings.showGifTrimmer {
                    ...
                    if let capturedMouseClickData,
                       ...
                       !capturedMouseClickData.events.isEmpty {
                        gifData = overlayMouseClicksOnGif(
                            gifData: gifData,
                            region: capturedMouseClickData.region,
                            events: capturedMouseClickData.events
                        )
                    ...
                    if let capturedMouseClickData,
                       capturedMouseClickData.type == .gif,
                       !capturedMouseClickData.events.isEmpty {
                        let gifData = try loadGifCaptureData(from: url)
                        let processedGifData = overlayMouseClicksOnGif(
                            gifData: gifData,
                            region: capturedMouseClickData.region,
                            events: capturedMouseClickData.events
                        )
    ...
    private func overlayMouseClicksOnVideo(
        sourceURL: URL,
        region: CaptureRegion,
    ...
    private func loadGifCaptureData(from url: URL) throws -> GifCaptureData {
    ...
    private func overlayMouseClicksOnGif(
    ...
    private func drawMouseClickPulseOverlay(
        on image: CGImage,
    ...
    private func mapMouseClickEvents(_ events: [MouseClickEvent], for region: CaptureRegion) -> [MappedMouseClickEvent] {
    ...
    private func capturePoint(for globalPoint: CGPoint, in region: CaptureRegion) -> CGPoint? {
...
private struct MouseClickEvent: Sendable {
...
@MainActor
private final class MouseClickMonitor {
